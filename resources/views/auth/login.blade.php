<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Login</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>

<body class="flex flex-col min-h-screen md:flex-row">

<div class="w-full md:w-1/2 flex items-center justify-center bg-gray-100 py-12 px-4">
    <div class="w-full max-w-sm">

        <h1 class="text-3xl font-bold mb-2 text-center">Selamat Datang!!!!</h1>
        <p class="text-gray-500 mb-6 text-center">Silakan lengkapi data diri Anda.</p>

        <form method="POST" action="{{ route('login') }}">
            @csrf

            <div class="mb-4">
                <label>Email</label>
                <input type="email" name="email" class="w-full p-3 rounded-lg border mt-1" placeholder="Masukkan email" required autofocus>
            </div>

            <div class="mb-4">
                <label>Password</label>
                <input type="password" name="password" class="w-full p-3 rounded-lg border mt-1" placeholder="********" required>
            </div>

            <div class="flex justify-between items-center text-sm mb-4">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="remember">
                    Ingat Saya
                </label>

                <a href="{{ route('password.request') }}" class="text-gray-500">
                    Lupa Password
                </a>
            </div>

            <button class="w-full text-white py-3 rounded-lg hover:bg-orange-600 transition" style="background-color: #FF9E0C;">
                Masuk
            </button>
         

<div class="mt-6">
    <div class="relative">
        <div class="absolute inset-0 flex items-center">
            <div class="w-full border-t border-gray-200"></div>
        </div>
        <div class="relative flex justify-center text-sm">
            <span class="px-2  text-gray-500">atau</span>
        </div>
    </div>

    <div class="mt-6">
        <a href="{{ route('google.login') }}" class="w-full flex items-center justify-center gap-3 bg-white border border-gray-300 text-gray-700 px-4 py-3 rounded-xl hover:bg-gray-50 transition-colors shadow-sm font-semibold">
            <svg class="w-5 h-5" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Masuk dengan Google
        </a>
    </div>
</div>
        </form>  

           @if($errors->any())
                <div class="text-red-500 mt-3">
                    {{ $errors->first() }}
                </div>
            
            @endif
        <p class="text-sm mt-4 text-center">
            Tidak punya akun?
            <a href="{{ route('register') }}" class="text-orange-500">
                Daftar gratis!
            </a>
        </p>

    </div>
</div>

<div class="w-full hidden md:w-1/2 md:flex items-center justify-center text-white p-10" style="background-color: #FF9E0C;">
    <div>
        <h1 class="text-4xl font-bold mb-4">
           Perjalanan Anda Berlanjut di Sini
        </h1>
        <p>
           Masuk untuk mengakses profil Anda, mengelola pesanan, dan memilih <br>armada premium Anda berikutnya.
        </p>
    </div>
</div>

</body>
</html>